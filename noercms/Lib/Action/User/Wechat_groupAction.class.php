...
class Wechat_groupAction extends UserAction{
    ...
    function groups(){
		...
		$count      = $product_model->where($where)->count();
        	...
        	$show       = $Page->show();
        	$list = $product_model->where($where)->order('id desc')->limit($Page->firstRow.',12')->select();
		*/
        $wechat_group_db = M('Wechat_group');
		$count      = $wechat_group_db->where(array('token' => $this -> token))->count();
		$Page       = new Page($count,12);
		$show = $Page->show();
        $groups = $wechat_group_db -> where(array('token' => $this -> token)) -> order('id ASC')->limit($Page->firstRow.',12') -> select();
