...
class JiudianAction extends UserAction{
	...
	public function infos(){
		...
		if(empty($check)){$this->error('请先填写好酒店管理页面再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
	...
	public function infos_detail(){
		if(IS_POST){
			$id = $this->_get('id');
			if($this->yuyue_order->where(array('id'=>$id))->save($_POST)!==false){
				$this->success('修改成功！',U($this->type.'/infos',array('pid'=>2,'token'=>$this->token)));
			}else{
				$this->error('修改失败！');
			}
		}else{
