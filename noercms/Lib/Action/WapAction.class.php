...
class WapAction extends BaseAction {
    ...
    protected function _initialize() {
        ...
        if (!$fansInfo) {
            $fansInfo = M('Userinfo')->where(array(
                'token' => $this->token,
                'wecha_id' => $this->wecha_id
            ))->find();
            $advanceInfo = M('Wechat_group_list')->where(array(
                'token' => $this->token,
        ...
        if (!$homeInfo) {
            $homeInfo = M('home')->where(array(
                'token' => $this->token
            ))->find();
            S('homeinfo_' . $this->token, $homeInfo);
        }
		//print_r($homeInfo);die();
        ...
        if (!$this->group) {
            $this->group = M('User_group')->where(array(
                'id' => intval($this->user['gid'])
            ))->find();
	...
	$.post('" . U('Share/shareData', array(
            'token' => $this->token,
            'wecha_id' => $this->wecha_id
        )) . "',submitData,function (data) {},'json')
}
        </script>";
