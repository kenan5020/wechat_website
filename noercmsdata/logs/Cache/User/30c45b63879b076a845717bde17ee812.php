    ...
    <div class="alert">
          ...
          <div class="cLine">
            <div class="pageNavigator left">
                <a href="<?php echo U('Reservation/add',array('token'=>$token,'thetype'=>'estate'));?>" class="btnGrayS vm bigbtn">
              ...
              <thead>
				  ...
				  <tbody>
					<?php if(is_array($reslist)): $i = 0; $__LIST__ = $reslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
						   ...
						   <td > 
								<a href="<?php echo U('Reservation/manage',array('token'=>$vo['token'],'thetype'=>'estate','id'=>$vo['id']));?>" class="btn" title="订单管理">订单管理</a>
								<a class="btn" href="<?php echo U('Reservation/edit',array('token'=>$vo['token'],'thetype'=>'estate','id'=>$vo['id']));?>" title="编辑">编辑</a> 
								<a class="btn" href="<?php echo U('Reservation/del',array('token'=>$vo['token'],'thetype'=>'estate','id'=>$vo['id']));?>" onclick="return(confirm('确定要删除吗？'))"  title="删除">删除</a></td>
